...
app.get('/business-classes', async (_, res) => {
  const businessClasses = await axios.get(`${API_URL}/business-classes`, {
    headers: HEADERS,
  });
...
app.get('/workers-compensation-classes-by-states', async (_, res) => {
  ...
  const statesPromises = states.map((state) => {
    return axios.get(`${API_URL}/workers-compensation-class-codes?state=${state}`, {
      headers: HEADERS,
    });
  });
